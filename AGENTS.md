...
- `bridge.py` contains the Telegram webhook bridge, tmux integration, and automatic webhook setup logic.
...
- State files in `CLAUDE_DIR` (default `~/.claude`):
  - `telegram_chat_id`: Current active chat ID
  - `telegram_pending`: Timestamp flag for Telegram-initiated messages
  - `telegram_webhook_url`: Last configured webhook URL (for change detection)
